...
### Changed
- Marketplace description and README intro rewritten to lead with the core feature ("Adds a numbered index to every cell in every notebook") so it's immediately clear what the extension does. Differentiators (custom templates, section-aware resets, `Ctrl+K G` cell picker, permalinks) follow in a single sentence. No code changes.

## [0.5.3] - 2026-04-28

### Changed
- Marketplace summary, README intro, and keyword list refreshed to reflect everything shipped in v0.5.1 (default `Ctrl+K G` keybinding, section-grouped cell picker) and v0.5.2 (Copy Cell Permalink command, `permalinkFormat` setting). No code changes.
